...
function DashboardView({ event, counts, total, confirmedCount, companions, respondedPct, filter, setFilter, query, setQuery, filtered, guests, guestLimit, onDelete, eventName, eventInfo, organizerName }: {
  ...
  const buildReportData = () => {
    ...
    return {
      ...
      guestLimit,
      confirmed: confirmedCount, companions,
...
function SettingsView({ event, guestLimit, setGuestLimit, eventName, setEventName, eventInfo, setEventInfo, colorId, setColorId, fontId, setFontId, trajeOn, setTrajeOn, trajeText, setTrajeText, acompOn, setAcompOn, restricaoOn, setRestricaoOn, msgOn, setMsgOn, msgText, setMsgText, onPersistInfo, onPersistAppearance, organizerName, userName, sidebarEvents, expiresAt, onReactivate }: {
  ...
  const inviteUrl = `${process.env.NEXT_PUBLIC_SITE_URL ?? (typeof window !== "undefined" ? window.location.origin : "https://vowify.netlify.app")}/convite/${event.id}`;
