...
export default function TermosPage() {
  return (
    <LandingProvider>
      <div style={{ minHeight: "100dvh", fontFamily: "var(--font-bricolage), system-ui, sans-serif" }}>

        {/* Header */}
        <header style={{ borderBottom: "1px solid var(--lp-border)", padding: "16px 24px" }}>
          <div style={{ maxWidth: 720, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <Link href="/" style={{ fontSize: 13, color: "var(--lp-t3)", textDecoration: "none" }}>← Voltar</Link>
          </div>
        </header>
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "56px 24px 96px" }}>
          ...
          <div style={{ marginBottom: 48 }}>
            ...
            <h1 style={{ fontSize: 40, fontWeight: 800, letterSpacing: "-0.03em", color: "var(--lp-t1)", margin: "0 0 12px", lineHeight: 1.1 }}>
              Termos de Uso
            </h1>
            <p style={{ fontSize: 15, color: "var(--lp-t3)", margin: 0, lineHeight: 1.6 }}>
              Última atualização: maio de 2026 · Leia com atenção antes de utilizar a plataforma Vowify.
            </p>
          </div>

          {/* Index */}
          <div style={{ background: "var(--lp-bg2)", borderRadius: 14, padding: "20px 24px", marginBottom: 48, border: "1px solid var(--lp-border)" }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: "var(--lp-t4)", textTransform: "uppercase", letterSpacing: ".06em", margin: "0 0 12px" }}>Índice</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {[
                ...
                <span key={item} style={{ fontSize: 14, color: "var(--lp-t2)" }}>{item}</span>
              ))}
            </div>
          </div>
          ...
          {/* Footer links */}
          <div style={{ marginTop: 56, paddingTop: 24, borderTop: "1px solid var(--lp-border)", display: "flex", gap: 24, fontSize: 13, flexWrap: "wrap" }}>
            <Link href="/privacidade" style={{ color: "#b14eff", textDecoration: "none" }}>Política de Privacidade →</Link>
            <Link href="/" style={{ color: "var(--lp-t3)", textDecoration: "none" }}>Voltar ao início</Link>
          </div>
        </div>
      </div>
    </LandingProvider>
...
function S({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 40 }}>
      <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--lp-t1)", margin: "0 0 12px", letterSpacing: "-0.015em" }}>{title}</h2>
      <p style={{ fontSize: 15, color: "var(--lp-t2)", lineHeight: 1.8, margin: 0, whiteSpace: "pre-line" }}>{children}</p>
    </div>
  );
}
