...
export function Features() {
  ...
  return (
    <section id="recursos" className="px-6 sm:px-8 lg:px-14 pb-24 max-w-[1280px] mx-auto w-full">
      <div ref={ref} className="vw-card" style={{ padding: 0, overflow: "hidden", background: "var(--lp-feat-gradient)" }}>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] items-center">
          ...
          >
            ...
            <h2
              ...
              style={{ lineHeight: 1.1, letterSpacing: "-0.025em", fontWeight: 600, margin: "12px 0 16px", color: "var(--lp-t1)" }}
            >
              Saiba{" "}
              <em style={{ fontStyle: "normal" }} className="vw-gradient-text">exatamente</em>{" "}
              quantos vão chegar.
            </h2>
            <p style={{ fontSize: 16, color: "var(--lp-t3)", lineHeight: 1.55, margin: "0 0 24px" }}>
...
function DashboardPreview({ animate }: { animate: boolean }) {
  ...
  return (
    <div className="vw-card vw-card-interactive" style={{ borderRadius: 16, overflow: "hidden", boxShadow: "0 20px 60px rgba(122,58,255,.18), 0 6px 18px rgba(15,11,30,.08)" }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--lp-border)", display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ width: 10, height: 10, borderRadius: 999, background: "#ff5f57" }} />
        <span style={{ width: 10, height: 10, borderRadius: 999, background: "#febc2e" }} />
        <span style={{ width: 10, height: 10, borderRadius: 999, background: "#28c840" }} />
        <div style={{ marginLeft: 12, fontSize: 12, color: "var(--lp-t3)" }}>vowify.app/eventos/marina-30</div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_1.2fr] gap-5 p-6">
        <div>
          <div style={{ fontSize: 13, color: "var(--lp-t3)", marginBottom: 4 }}>30 anos da Marina</div>
          <div style={{ fontSize: 22, fontWeight: 600, letterSpacing: "-0.02em", marginBottom: 16, color: "var(--lp-t1)" }}>Painel do evento</div>
          <Donut size={150} stroke={18} animate={animate} segments={[{ value: 38, color: "#b14eff" }, { value: 22, color: "#f59e0b" }, { value: 6, color: "#e1124e" }]}>
            <span style={{ fontSize: 28, fontWeight: 600, color: "var(--lp-t1)", opacity: animate ? 1 : 0, transition: "opacity 0.4s ease 0.65s" }}>58%</span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ fontSize: 12, fontWeight: 500, color: "var(--lp-t3)", marginBottom: 4 }}>Últimas confirmações</div>
          {activity.map(([n, t, s]) => (
            <div key={n} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0" }}>
              <Avatar name={n} size={26} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: "var(--lp-t1)" }}>{n}</div>
                <div style={{ fontSize: 11, color: "var(--lp-t3)" }}>{t}</div>
              </div>
...
function DonutRow({ dot, label, value }: { dot: string; label: string; value: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ flex: 1, color: "var(--lp-t3)" }}>{label}</span>
      <span style={{ fontWeight: 600, fontFamily: "var(--font-geist-mono), monospace", color: "var(--lp-t1)" }}>{value}</span>
    </div>
  );
}
